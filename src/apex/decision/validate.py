...
from apex.domain.rules import validate_bench_order, validate_squad, validate_xi

def validate_system_decision(official: OfficialSnapshot, decision: SystemDecision) -> tuple[str, ...]:
    players = official.player_map()
    errors = list(validate_squad(players, decision.squad_ids, budget_tenths=None))
    errors += validate_xi(players, decision.squad_ids, decision.xi_ids)
    errors += validate_bench_order(players, decision.squad_ids, decision.xi_ids, decision.bench_order)
