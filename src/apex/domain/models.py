...
class OfficialSnapshot:
    ...
    def decision_universe(
        self,
        owned_player_ids: set[int] | frozenset[int] = frozenset(),
    ) -> frozenset[int]:
        # Provider coverage must never redefine the legal FPL universe. Include every
        # Official element that is transaction-eligible plus any currently owned
        # player that mechanics may need to retain/sell.
