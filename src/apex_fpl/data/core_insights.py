...
RAW_BASE = "https://raw.githubusercontent.com/olbauday/FPL-Core-Insights/main/data/{season}"


class FPLCoreClient:
    def __init__(self, http: CachedHttp, season: str):
        self.http = http
        self.season = season

    def _csv(self, name: str, force: bool = False) -> pd.DataFrame:
        url = f"{RAW_BASE.format(season=self.season)}/{name}"
        text = self.http.get_text(url, f"core_{self.season}_{name.replace('/', '_')}", force)
    ...
    def preseason_friendlies(self, force: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            return pd.DataFrame(columns=["player_id", "match_id", "minutes_played", "xg", "xa", "defensive_contributions", "start_min"])
