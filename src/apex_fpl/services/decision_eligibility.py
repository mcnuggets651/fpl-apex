...
def source_health_status(sources: list) -> dict:
    row = next(
        (s for s in sources if getattr(s, "name", "") == "news_source_health"),
        None,
    )
    try:
...
def _decision_grade(events: pd.DataFrame) -> bool:
    if events.empty:
        return False
    if events["source_tier"].astype(str).isin(
        {"official_club", "official_league"}
    ).any():
        return True
    return bool(
...
def _fresh_specialist_predictions(
    ...
    *,
    ...
    frame = current_evidence_rows(
        ...
        now=now,
        label="governed specialist evidence",
        strict=True,
    )
    if frame.empty:
        return frame
    ...
    if not identity.ready:
        raise ValueError(
            "specialist pre-solve identity failed: "
...
def _fresh_squad_hierarchy(
    ...
    *,
    ...
    required = {"player_id", "hierarchy_status", "checked_at", "valid_until"}
    if not path.exists():
    ...
    frame, identity = resolve_source_identities(
        players,
        frame,
        source="squad_hierarchy_presolve",
    ...
    if not identity.ready:
        raise ValueError(
            "squad hierarchy pre-solve identity failed: "
            + "; ".join(identity.blockers[:10])
        )
...
def evidence_eligibility(
    ...
    *,
    ...
    squad_hierarchy_path: Path | None = None,
    now: datetime | None = None,
    ...
    hierarchy_path = (
        Path(squad_hierarchy_path)
        if squad_hierarchy_path is not None
        else DEFAULT_SQUAD_HIERARCHY
    )
    hierarchy = _fresh_squad_hierarchy(
        out,
        hierarchy_path,
        now=now,
        strict_identity=squad_hierarchy_path is not None,
    )
        ...
        for row in hierarchy.itertuples(index=False):
            ...
            if status in WEAK_SQUAD_HIERARCHY:
                ...
                out.loc[mask, "evidence_state"] = (
                    "authoritative_weak_squad_hierarchy"
                )
                reasons.setdefault(pid, []).append(
                    f"current governed squad hierarchy is {status}; "
                    "excluded from production squad pre-solve"
                )
    ...
    if not specialist_report.empty:
        for row in specialist_report.itertuples(index=False):
            ...
            if (
                ...
                mask = out["player_id"].astype(int).eq(pid)
                model_start = pd.to_numeric(
                    out.loc[mask, "start_probability"], errors="coerce"
                ).fillna(0.0)
                material_contradiction = bool(
                    (
                        model_start
                        >= MATERIAL_SPECIALIST_CONTRADICTION_START_PROBABILITY
                    ).any()
                )
                if material_contradiction:
                    ...
                    reasons.setdefault(pid, []).append(
                        "fresh two-source governed non-start consensus materially "
                        "contradicts model start probability; XI eligibility removed pre-solve"
                    )
                else:
                    out.loc[mask, "evidence_state"] = "specialist_nonstart_diagnostic"
                    reasons.setdefault(pid, []).append(
                        "fresh two-source governed non-start consensus recorded below "
                        "hard-constraint contradiction threshold"
                    )
    ...
    report = {
        ...
        "fresh_hierarchy_evidence_ids": sorted(
            pd.to_numeric(hierarchy.get("player_id"), errors="coerce")
            .dropna()
            .astype(int)
            .unique()
            .tolist()
        ...
        "fresh_specialist_evidence_ids": sorted(
            pd.to_numeric(specialist.get("player_id"), errors="coerce")
            .dropna()
            .astype(int)
            .unique()
            .tolist()
