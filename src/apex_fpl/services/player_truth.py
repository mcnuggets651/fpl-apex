...
def _pair_coverage(projections: pd.DataFrame, player_ids: set[int], gameweeks: list[int], *, presence_column: str | None = None) -> tuple[float, dict[int, int], list[tuple[int, int]]]:
...
def audit_player_truth(players: pd.DataFrame, projections: pd.DataFrame, expected_players: int | None) -> dict:
    ...
    projection_coverage, projection_counts, missing_projection_pairs = _pair_coverage(projections, valid_ids, gameweeks)
    ...
    if absent_pairs:
        ...
        if missing_absence_cols:
            blockers.append(f"AIrsenal source absence is not explicitly reconciled; missing columns={missing_absence_cols}")
        else:
            keyed = projections.copy()
            keyed["player_id"] = pd.to_numeric(keyed["player_id"], errors="coerce")
            keyed["gw"] = pd.to_numeric(keyed["gw"], errors="coerce")
            keyed = keyed.dropna(subset=["player_id", "gw"])
            keyed["player_id"] = keyed["player_id"].astype(int)
            keyed["gw"] = keyed["gw"].astype(int)
            grouped = keyed.groupby(["player_id", "gw"], as_index=False).agg(
                source_absent=("airsenal_source_absent", "all"),
                fallback_weight=("effective_weight_airsenal_fallback_apex", "sum"),
                fallback_xp=("xp_expert_airsenal_fallback_apex", "sum"),
            )
    ...
    if not projections.empty and {"player_id", "xp_set_piece_prior"}.issubset(projections.columns):
        projection_set_piece = projections.assign(xp_set_piece_prior=pd.to_numeric(projections["xp_set_piece_prior"], errors="coerce").fillna(0.0)).groupby("player_id", as_index=False)["xp_set_piece_prior"].sum().rename(columns={"xp_set_piece_prior": "set_piece_xp_horizon"})
    ...
    for idx, row in players.iterrows():
        ...
        explicit = {field: (None if pd.isna(row.get(field)) else float(row.get(field))) for field in SHARE_FIELDS}
        ...
        verified_share = bool(has_explicit_share and source_tier in TRUSTED_SOURCE_TIERS and source_url.startswith(("https://", "http://")) and evidence_type)
        ...
        if not projection_set_piece.empty and player_id >= 0:
            match = projection_set_piece[pd.to_numeric(projection_set_piece["player_id"], errors="coerce").eq(player_id)]
            if not match.empty:
        ...
        rows.append({
            ...
            "airsenal_source_absent_gameweeks": len(gameweeks) - int(airsenal_counts.get(player_id, 0)),
            ...
            **explicit,
            "set_piece_share_source": "verified_current_override" if verified_share else "unverified" if has_explicit_share else "none",
    ...
    if unverified_share_ids:
        blockers.append("explicit set-piece shares without trusted current provenance: " + ",".join(map(str, sorted(set(unverified_share_ids))[:30])))
    if unexplained_set_piece_xp_ids:
        blockers.append("set-piece xP exists without verified explicit share evidence: " + ",".join(map(str, sorted(set(unexplained_set_piece_xp_ids))[:30])))
    if inferred_roles:
        warnings.append(f"{inferred_roles}/{len(players)} tactical roles are statistical inference; this is forecast uncertainty, not a factual-data failure")
