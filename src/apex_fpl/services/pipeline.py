...
def _status(name: str, ok: bool, detail: str = "", configured: bool = True, version: str = "") -> SourceStatus:
    return SourceStatus(name=name, ok=ok, detail=detail, configured=configured, version=version)
...
def run_pipeline(
    ...
    sources.append(_status(
        "official_fpl", True,
        f"{len(official.players)} players; {len(official.fixtures)} fixtures; snapshot={manifest['snapshot_id']}",
    ...
    try:
        ...
        sources.append(_status("fpl_core_playerstats", True, f"{len(core)} rows", version=core_pin))
        try:
            friendlies = core_client.preseason_friendlies(force=force)
            sources.append(_status("fpl_core_preseason", True, f"{len(friendlies)} player-match rows", version=core_pin))
    ...
    if not manual.empty:
        ...
        sources.append(_status("manual_availability", True, "not configured", configured=False))
    ...
    if not tactical.empty:
        ...
        if unknown:
            raise ValueError(f"tactical role file contains unknown official FPL IDs: {unknown[:10]}")
        players = players.merge(tactical, on="player_id", how="left")
        sources.append(_status("tactical_roles", True, f"{len(tactical)} verified role overrides"))
    else:
        sources.append(_status("tactical_roles", True, "no verified overrides", configured=False))
    role_mult = players["role_multiplier"] if "role_multiplier" in players else pd.Series(1.0, index=players.index)
    role_conf = players["role_confidence"] if "role_confidence" in players else pd.Series(0.65, index=players.index)
    ...
    try:
        ...
        if not air.empty:
            ...
            covered = sorted(set(air["gw"]) & set(gws))
            missing = sorted(set(gws) - set(covered))
            ok = not missing
            sources.append(_status(
                "airsenal", ok,
                f"{len(air)} projection rows; covered GW={covered}" + (f"; missing GW={missing}" if missing else ""),
                configured=True, version=air_pin,
            ))
        else:
            sources.append(_status("airsenal", True, "genuine projection export not configured", configured=False, version=air_pin))
    except Exception as exc:
        sources.append(_status("airsenal", False, str(exc), configured=bool(settings.airsenal_csv), version=air_pin))
    ...
    try:
        ...
        if not odds.empty:
            ...
            sources.append(_status("market_odds", True, "optional endpoint not configured", configured=False))
    except Exception as exc:
    ...
    horizon_vals = proj.groupby("player_id", as_index=False).agg(horizon_xp=("weighted_xp", "sum"))
    first = proj[proj["gw"] == gws[0]][["player_id", "risk_adjusted_xp"]].rename(columns={"risk_adjusted_xp": "gw1_xp"})
    ranked = players.merge(horizon_vals, on="player_id", how="left").merge(first, on="player_id", how="left").merge(summaries, on="player_id", how="left")
    for col in ["horizon_xp", "gw1_xp", "xpts_1", "xpts_3", "xpts_5", "xpts_8", "projection_confidence"]:
    ...
    if scenario in {"haaland", "both"} and haaland_id is not None:
        scenarios["haaland"] = optimise_squad(ranked, settings.budget, settings.max_per_team, locked={haaland_id})
    if scenario in {"no-haaland", "both"} and haaland_id is not None:
        scenarios["no-haaland"] = optimise_squad(ranked, settings.budget, settings.max_per_team, banned={haaland_id})
    ...
    if plan_transfers:
        ...
        if team_state is not None:
            transfer_plan = optimise_transfer_plan(
                ranked, proj, gws, team_state.squad, bank=team_state.bank,
                free_transfers=team_state.free_transfers, max_per_team=settings.max_per_team,
                decay=settings.fixture_decay,
            )
            sources.append(_status("team_state", True, "manual current squad loaded"))
        else:
            sources.append(_status("team_state", True, "not configured; initial-squad mode", configured=False))

    safety = assess_safety(
        official, sources, integrity, proj, scenarios,
        settings.required_sources, settings.max_official_age_hours,
    )

    ranked_cols = [
        "player_id", "web_name", "team_name", "position", "price", "status",
        "chance_of_playing_next_round", "expected_minutes", "start_probability",
        "appearance_probability", "minutes_60_plus_probability", "minutes_confidence",
        "tactical_role", "role_confidence", "preseason_minutes", "preseason_starts",
        "preseason_xg90", "preseason_xa90", "gw1_xp", "xpts_1", "xpts_3", "xpts_5",
        "xpts_8", "horizon_xp", "projection_confidence",
    ]
    ranked_out = ranked[[c for c in ranked_cols if c in ranked]].sort_values("horizon_xp", ascending=False)
    write_reports(
        settings.report_dir, ranked_out, proj, integrity, news_audit, scenarios,
        transfer_plan, sources, gws, safety=safety, snapshot=manifest, upstreams=pins,
    )
    return PipelineOutput(ranked_out, proj, integrity, news_audit, scenarios, transfer_plan, sources, gws, safety, manifest)
