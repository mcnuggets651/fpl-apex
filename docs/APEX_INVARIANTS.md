...
- **INV-PRODUCTION-CERTIFICATE-ONLY** — V2 publication eligibility derives only from the complete constitutional AssuranceCase, its blocker-free ReleaseCertificate and an immutable proof-derived ProductionPublicationAuthorization; a caller cannot supply or override readiness/safety.
- **INV-PRODUCTION-BACKEND-QUALIFIED** — production requires retained evidence that ArtifactStore and ReleaseRegistry are durable shared control-plane backends with immutable history and atomic compare-and-swap; filesystem reference adapters are not production-qualified by default.
...
- **INV-PRODUCTION-REPLAY-EXACT** — ProductionPublicationAuthorization, ProductionCutoverReport and ReleaseRecord evidence are content-addressed and replay re-derives the AssuranceCase certificate, constitutional proof surface, backend qualification, source artifacts and release identity exactly.
