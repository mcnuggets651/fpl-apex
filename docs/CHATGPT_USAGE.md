...
Apex V2 frozen engine SHA is `99cc7b51b0cff45462b567084cb1844cfe0a456f`. The sole serving production workflow is `.github/workflows/apex-v2-daily-production.yml`; **AIrsenal** is the sole serving provider H1–H8. Research output is non-serving and has `production_influence = NONE`.

## What ChatGPT should read first

For an Apex engineering or recommendation task:

1. `docs/APEX_V2_AUTHORITY.json` — machine-readable production constitution.
...
6. Live GitHub `main`, PR #90 and the relevant immutable Apex V2 release/workflow state.
...
A current Apex production verdict requires an immutable final produced by the authenticated frozen Apex V2 production chain for entry **63984**. Verify:

- frozen engine SHA and current operations control-plane identity;
...
- immutable final publication identity and freshness.
...
- **Apex production verdict** — immutable result from `.github/workflows/apex-v2-daily-production.yml`.
- **operations** — auth, deadline dispatch and evaluation controllers around the frozen engine.
