...
3. For an Apex production verdict, use the current authenticated immutable production result for entry 63984 generated by the exact authority-declared serving core; never reconstruct current manager state from memory or historical generated files.
...
If exact authenticated production state, serving projections, serving-core provenance, factual authority or immutable publication is unsafe, say exactly what is missing and withhold an invented Apex production recommendation. Do not fall back to retired Pinnacle/Elite outputs or choose manually among shadow candidates.

## Canonical wording

Use `Apex production verdict` only for the current immutable result produced by `.github/workflows/apex-v2-daily-production.yml` under the exact authority-declared `production_core_sha`. Use `operations`, `shadow/research`, `historical`, or `external gap-fill` for other evidence.
