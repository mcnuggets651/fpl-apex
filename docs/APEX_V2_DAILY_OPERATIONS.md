...
The independently governed serving-code pointer is `production_core_sha` in `APEX_V2_AUTHORITY.json`. A production core must be a 40-character commit SHA descending from the immutable PR #90 base and must pass the full successor/readiness/canary gate before that pointer moves. The authority split and first hardened-core promotion are complete: the immutable base remains `99cc...`, while current serving code is the separate authority-declared production core above.
