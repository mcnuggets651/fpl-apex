...
The public Apex repository may publish only the explicit public allowlist produced by `runtime/publication.py`: public attempt metadata, the canonical Apex serving forecast, provider provenance/hashes, public governance, public evidence and attestations. The compatibility `provider_forecasts.tar.gz` asset is provenance-only and must never contain provider forecast rows or expected-points exports. Recursive snapshot publication is forbidden. Raw `TeamState`, exact purchase/selling prices, bank, free transfers, pending chip state and personalized `SystemDecision` are `PRIVATE_MANAGER` and must never be assets of a public release or public Actions diagnostic artifact.
