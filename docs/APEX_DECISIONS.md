...
## D034 — Backend production qualification is two-plane and runtime-verifier-only

Apex must never infer real production durability from a green database integration test. Backend admission therefore has two independent evidence planes. Plane A is fresh-adapter mechanical evidence for persisted identity, shared visibility, integrity, immutable replay and atomic stale-writer-safe CAS. Plane B is retained deployment/operations evidence for retention, access control, credential separation, backup, restore, disaster recovery, availability and geographic durability. Plane B must be complete, `PRODUCTION`-class, PASS, backed by retained source artifacts and bound to the exact ArtifactStore identity, ReleaseRegistry identity and qualification scope. GitHub Actions PostgreSQL, TEST-class evidence, synthetic fixtures, random artifacts, copied evidence and caller-authored booleans cannot qualify production. Both role-specific production qualification bindings must reference the same Plane-B evidence. Production cutover and answer-authority paths are verifier-only for Plane B: they replay retained qualification and actual backend identity but do not self-author operational PASS evidence. Reference filesystem adapters remain structurally non-production.

## D035 — Prospective chronology and qualification are operator-enforced, promotion remains separate

The pure V2 empirical contracts remain replayable domain primitives, but production operators must use a narrower fail-closed adapter. Production backend selection requires an explicitly configured PostgreSQL control plane and has no filesystem fallback. Forecast-model and DecisionPolicy candidate availability is recorded at materialization time and begins in SHADOW. Experiment declaration time is recorded by the operator process and must precede the evaluation window; result availability is recorded by the operator process and cannot precede the window end. Caller-supplied backdating fields are not accepted. Qualification is derived only from retained candidate/definition/registry/result lineage and can produce a reviewable QUALIFIED candidate proposal only for an exact SUPPORTED certificate. It does not edit a champion. Champion admission remains a separate immutable reviewed registry/promotion operation. Already-known historical outcomes may be useful for archaeology/pseudo-prospective analysis but cannot be inserted through this path and called prospective production evidence.

## D036 — Qualification does not confer champion authority

A SHADOW or QUALIFIED candidate is never the production champion merely because its qualification artifact exists or a registry/configuration row names it. Forecast-model champion authority is inherited only from the existing learning-governance chain and must be independently re-derived at replay: exact retained candidate/incumbent production evaluations, common-truth lineage, exact comparison rows and the retained qualified champion learning-policy registry must reproduce the promotion rules and still derive the exact `ModelPromotionCertificate(PROMOTE)` bound into the `ModelRegistryGeneration`. A hand-authored `PROMOTE` label is insufficient. DecisionPolicy, scenario-generator and scenario-policy authority each require exact typed empirical qualification plus a separate immutable reviewed `ChampionAdmissionCertificate` with retained review evidence. Review and generation authorization timestamps are timezone-aware and point-in-time; future review/authorization cannot leak backward. The four authorities are composed into one parent-linked `ProductionChampionGeneration`; stale writers cannot create the next generation without the exact expected parent. Production cutover and answer authority are verifier-only for this chain: they independently replay the generation and exact-match all four identities against the schema-v2 production planning bundle. Runtime publication code cannot issue admissions, create generations or silently fall back to repository configuration. Missing, corrupt, not-yet-valid, expired, wrong-season, forged-promotion or mismatched champion authority withholds publication. Synthetic authority fixtures prove mechanics only and never constitute a real production champion.
