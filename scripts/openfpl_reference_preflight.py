...
def main() -> None:
    ...
    plug = (
        tuple(
            line.strip()
            for line in (openfpl_root / "plug.txt")
            .read_text(encoding="utf-8")
            .splitlines()
            if line.strip()
        )
        if (openfpl_root / "plug.txt").is_file()
        else ()
    )
    ...
    missing_markers = [
        marker for marker in required_notebook_markers if marker not in notebook
    ]
    if missing_markers:
        errors.append(
    ...
    reproducibility = reference_reproducibility(openfpl_root)
    if (
        reproducibility["reference_inference_state"]
    ...
    report = {
        ...
        "ensemble_contract": (
            "five-fold position-specific models; median across loaded candidate predictions"
        ),
        **reproducibility,
