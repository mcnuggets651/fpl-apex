...
replace_once(
    ...
    '''    out = out.merge(rates, on="player_id", how="left", validate="one_to_one")\n    return out\n''',
    '''    out = out.merge(rates, on="player_id", how="left", validate="one_to_one")\n    out["understat_player_matched"] = (\n        pd.to_numeric(out.get("understat_xg90"), errors="coerce").notna()\n        & pd.to_numeric(out.get("understat_xa90"), errors="coerce").notna()\n    )\n    out["understat_match_method"] = out.get(\n        "understat_match_method", pd.Series(pd.NA, index=out.index, dtype="object")\n    )\n    return out\n''',
)
