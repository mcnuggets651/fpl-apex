...
def parity_shadow(projections: pd.DataFrame, players: pd.DataFrame):
    ...
    audit["evidence_minutes"] = s[["xg90_combined_effective_evidence_minutes", "xa90_combined_effective_evidence_minutes"]].max(axis=1).to_numpy()
    ...
    ratio = np.divide(new_score, old_score, out=np.ones(len(out)), where=old_score.to_numpy()>1e-12)
    old_attack = num(out, "xp_attack")
    shrunk_apex = (num(out,"apex_xp") - old_attack + old_attack*ratio).clip(lower=0.0)
    out["shrunk_blended_xp_v2"] = (num(out,"xp") - num(out,"xp_expert_apex_model") + shrunk_apex*num(out,"effective_weight_apex_model")).clip(lower=0.0)
    return audit, out
