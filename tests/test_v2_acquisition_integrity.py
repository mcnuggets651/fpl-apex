...
def _record(text: str) -> EvidenceRecord:
...
def _config(tmp: Path, provider: bool = False) -> Path:
    p = tmp / "apex_v2.yaml"
    providers = "providers: []\n"
    if provider:
...
def _write_evidence(tmp: Path, records=()):
    ...
    manifest = {"schema_version": 1, "completed": True, "observed_official_hash": "stable-hash", "target_gameweek": 2, "record_count": len(records), "records_sha256": _records_hash(records), "source_config_sha256": hashlib.sha256(sources.read_bytes()).hexdigest(), "required_source_failures": []}
...
def _patch_common(monkeypatch, tmp: Path):
    monkeypatch.setattr(acquire_module, "fetch_official_snapshot", lambda **_: (_official(), {"bootstrap": {}, "fixtures": []}))
    monkeypatch.setattr(acquire_module, "acquire_team_state", lambda *a, **k: _team())
    _write_evidence(tmp)


def test_manifest_rejects_same_count_evidence_substitution(tmp_path: Path):
    original = (_record("Player is ruled out."),)
    _write_evidence(tmp_path, original)
    tampered = (_record("Player is fully available."),)
...
def test_required_evidence_source_config_is_sealed(monkeypatch, tmp_path: Path):
    ...
    snapshot = acquire_module.acquire_and_freeze(_config(tmp_path), run_id="source-seal", code_sha="abc", run_started_at="2026-08-28T11:59:00+00:00", workdir=tmp_path, expected_official_hash="stable-hash")
    ...
    assert snapshot.read_json("run.json")["evidence_source_config_sha256"] == manifest["source_config_sha256"]
...
def test_provider_raw_mutation_during_load_aborts(monkeypatch, tmp_path: Path):
    ...
    with pytest.raises(acquire_module.AcquisitionStageError) as err:
        acquire_module.acquire_and_freeze(_config(tmp_path, True), run_id="provider-toctou", code_sha="abc", run_started_at="2026-08-28T11:59:00+00:00", workdir=tmp_path, expected_official_hash="stable-hash")
