...
def test_production_cutover_has_no_network_v1_runtime_or_filesystem_backend_shortcut() -> None:
    ...
    assert "verify_production_authority_closure(" in text
    assert "_legacy.execute_production_cutover(" in text
    assert "derive_release_certificate" in transaction
    assert "production_registry.compare_and_swap_current" in transaction
    assert "ProductionPublicationAuthorization" in transaction
    assert "_validate_backend_binding" in transaction
    assert "PRODUCTION_PROOF_CLASSES" in transaction
    assert "load_empirical_qualification_certificate" in transaction
...
def test_production_cutover_requires_schema_v2_planning_bundle_authority() -> None:
    ...
    assert "VerifiedProductionPlanningBundle" in planning
...
def test_production_reference_solver_proof_is_planning_bound_and_replay_validated() -> None:
    ...
    assert "REFERENCE_SOLVER_PARITY_PROOF_ID" in verifier
    assert "load_planning_reference_solver_certificate" in verifier
    assert "load_reference_solver_authorization" in verifier
    assert "claim_has_matching_planning_reference_solver_parity" in transaction
...
def test_production_cutover_accepts_no_independent_readiness_or_safety_input() -> None:
    ...
    transaction = CUTOVER_TRANSACTION.read_text(encoding="utf-8")
    assert "_legacy.execute_production_cutover(" in source
    assert "ready_to_act=publishable" in transaction
    assert "safe_to_act=publishable" in transaction
...
def test_production_authorization_binds_release_validity_window() -> None:
    ...
    assert "production authority root does not cover the full release validity horizon" in cutover
    assert "record.created_at != authorization.created_at" in transaction
    assert "record.valid_until != authorization.valid_until" in transaction
