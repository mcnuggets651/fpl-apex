...
def test_two_specialists_agreeing_against_selected_player_blocks_until_resolved():
    specialist = pd.DataFrame(
        [{"player_id": 1, "review_priority": "high", "review_reason": "two specialist predicted-XI sources expect a benching"}]
    )
    result = audit_selected_squad_reality(
...
def test_high_transfer_risk_blocks_selected_player_without_mutating_projection():
    ...
    transfer = pd.DataFrame(
        [{"player_id": 1, "review_priority": "high", "review_reason": "agreement/medical-level transfer report requires official resolution"}]
    )
    result = audit_selected_squad_reality(
...
def test_neave_shape_cannot_be_published_as_first_bench():
    players = _players()
    players.loc[players.player_id.eq(2), ["web_name", "expected_minutes", "appearance_probability", "start_probability"]] = [
