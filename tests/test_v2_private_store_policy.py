...
def test_private_store_policy_accepts_private_immutable_repository():
    session = FakeSession(
        [
            FakeResponse(200, {"private": True}),
            FakeResponse(200, {"enabled": True, "enforced_by_owner": False}),
        ]
    )
    store = GitHubReleaseStore("owner/apex-private", "token", session=session)
    policy = store.assert_repository_policy(require_private=True, require_immutable=True)
...
def test_private_store_policy_rejects_public_repository_before_immutability_check():
