...
class _Result:
    ...
    note: str = "base"
    convergence_rank_prefixes: tuple[int, int] | None = None
...
def test_current_production_note_certificate_avoids_duplicate_expensive_solve() -> None:
    """Regression: joint_initial_path emits 'is checked', not 'was checked'."""
    calls: list[int] = []
    ...
    stable = _Result(
        ...
        note=(
            "Candidate convergence is checked between rank prefixes 32 and 48; "
            "rank 64 is solved only after another identity change."
        ),
    )
...
def test_legacy_was_checked_certificate_remains_compatible() -> None:
    calls: list[int] = []
    stable_ids = tuple(range(101, 116))
    stable = _Result(
        selected=_Selected(squad_ids=stable_ids),
...
def test_structured_certificate_is_authoritative_when_present() -> None:
    calls: list[int] = []
    ...
    def optimiser(*args, **kwargs):
        ...
        return _Result(
            ...
            note="Candidate convergence is checked between rank prefixes 32 and 48.",
            convergence_rank_prefixes=(32, 48),
        )
...
def test_malformed_structured_certificate_fails_closed_without_prose_rescue() -> None:
    calls: list[int] = []
    ids = tuple(range(101, 116))
    ...
    def optimiser(*args, **kwargs):
        ...
        return _Result(
            ...
            full_pool_selected_ids=ids,
            note="Candidate convergence is checked between rank prefixes 32 and 48.",
            convergence_rank_prefixes=(32, 48.5),  # type: ignore[arg-type]
        )
...
def test_certificate_must_cover_historical_broader_requirement() -> None:
    ...
    def optimiser(*args, **kwargs):
        ...
        return _Result(
            ...
            small_pool_selected_ids=ids,
            full_pool_selected_ids=ids,
            convergence_rank_prefixes=(16, 20),
        )
