...
def _module():
    ...
    spec = importlib.util.spec_from_file_location("audit_player_identity_script_contract", path)
...
def _official() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {"player_id": 1, "web_name": "Raya", "first_name": "David", "second_name": "Raya", "team": 1, "team_name": "Arsenal", "position": "GK"},
            {"player_id": 2, "web_name": "João Pedro", "first_name": "João", "second_name": "Pedro", "team": 2, "team_name": "Chelsea", "position": "FWD"},
            {"player_id": 3, "web_name": "Guéhi", "first_name": "Marc", "second_name": "Guéhi", "team": 3, "team_name": "Man City", "position": "DEF"},
        ]
    )
...
def _args(tmp_path: Path, airsenal: Path, recommendation: Path | None = None) -> argparse.Namespace:
...
def test_sealed_audit_accepts_complete_roster_and_records_provenance(tmp_path, monkeypatch):
    module = _module()
    ...
    recommendation.write_text(
        json.dumps({"squad_ids": [1, 2, 3], "captain_id": 2, "vice_captain_id": 1}),
        encoding="utf-8",
    )
...
def test_sealed_audit_fails_closed_when_airsenal_roster_is_incomplete(tmp_path, monkeypatch):
    module = _module()
    _write_manifest(tmp_path)
    airsenal = tmp_path / "airsenal.csv"
    ...
    assert any("missing 1 Official FPL player IDs" in blocker for blocker in audit["blockers"])
...
def test_sealed_audit_rejects_unknown_selected_player_id(tmp_path, monkeypatch):
    ...
    _write_airsenal(airsenal, [1, 2, 3])
    recommendation = tmp_path / "recommendation.json"
    recommendation.write_text(json.dumps({"squad_ids": [1, 2, 999]}), encoding="utf-8")
    monkeypatch.setattr(module.DecisionBundle, "load", lambda _: _Bundle())
    ...
    assert any("recommendation references unknown" in blocker for blocker in audit["blockers"])


def test_main_persists_machine_readable_failure_when_bundle_load_raises(tmp_path, monkeypatch):
    module = _module()
    bundle_dir = tmp_path / "broken-bundle"
    bundle_dir.mkdir()
    airsenal = tmp_path / "airsenal.csv"
    airsenal.write_text("player_id,source_player_name,identity_witness_type\n", encoding="utf-8")
